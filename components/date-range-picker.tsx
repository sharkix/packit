...
export function DateRangePicker({
  ...
  for (let i = 0; i < startDow; i++) {
    cells.push({
      date: new Date(viewYear, viewMonth, i - startDow + 1),
      other: true,
    })
  }
  ...
  return (
    <div>
      <span className="mb-1.5 block text-sm font-semibold">Termín cesty</span>
      <div className="grid grid-cols-2 gap-2">
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
          aria-expanded={open}
          className="flex items-center gap-2 rounded-lg border border-input bg-card px-3 py-3 text-left transition-colors focus:border-ring focus:outline-none focus:ring-2 focus:ring-ring/30"
        >
          <CalendarDays
          ...
          <span className="min-w-0">
            ...
            <span className="block truncate text-sm font-medium">
              {startDate ? fmtShort(startDate) : 'Vybrať dátum'}
            </span>
          </span>
        </button>
        <button
        ...
        >
          ...
          />
          <span className="min-w-0">
            <span className="block text-xs text-muted-foreground">Do</span>
            <span className="block truncate text-sm font-medium">
              {endDate ? fmtShort(endDate) : 'Vybrať dátum'}
            </span>
          </span>
        </button>
      </div>
        ...
        <div className="mt-2 rounded-lg border border-border bg-card p-4 shadow-lg">
          ...
          <div className="grid grid-cols-7 gap-y-1 text-center">
            ...
            {cells.map(({ date, other }, i) => {
              ...
              if (ds === todayStr && !isStart && !isEnd)
                cls += 'ring-1 ring-primary/50 '
          ...
          <div className="mt-3 flex items-center justify-between border-t border-border pt-3">
            <span className="text-sm text-muted-foreground">
              {startDate && endDate
                ? `${fmtShort(startDate)} – ${fmtShort(endDate)} · ${nights + 1} ${
                    nights + 1 === 1 ? 'deň' : nights + 1 <= 4 ? 'dni' : 'dní'
                  }`
                : startDate
                  ? 'Vyber dátum návratu'
                  : 'Vyber dátum odchodu'}
            ...
            <button
              ...
              className="rounded-md bg-primary px-4 py-1.5 text-sm font-semibold text-primary-foreground transition-opacity disabled:opacity-40"
            >
              Hotovo
            </button>
          </div>
      ...
      <div className="mt-2 grid grid-cols-2 gap-2">
        <div>
          ...
          <input
            ...
            value={startTime}
            onChange={(e) =>
              onChange({ startDate, endDate, startTime: e.target.value, endTime })
            }
            className="w-full rounded-lg border border-input bg-card px-3 py-2 text-sm outline-none transition-colors focus:border-ring focus:ring-2 focus:ring-ring/30"
          />
        </div>
        <div>
          <label
          ...
          <input
            ...
            value={endTime}
            onChange={(e) =>
              onChange({ startDate, endDate, startTime, endTime: e.target.value })
            }
            className="w-full rounded-lg border border-input bg-card px-3 py-2 text-sm outline-none transition-colors focus:border-ring focus:ring-2 focus:ring-ring/30"
          />
        </div>
